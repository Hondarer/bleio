...
espio/
├── server/
│   └── espio_server/      ESP32 ファームウェアプロジェクト
│       ├── src/           ソースコード (main.c)
│       ├── include/       ヘッダファイル
│       ├── lib/           ライブラリ
│       ├── test/          テストコード
...
| 1 | SET_OUTPUT | ピンを出力モードに設定する |
| 2 | SET_INPUT_PULLUP | ピンを内部プルアップ抵抗付き入力モードに設定する |
| 10 | WRITE_LOW | ピンの出力を LOW (0V) に設定する |
| 11 | WRITE_HIGH | ピンの出力を HIGH (3.3V) に設定する |

**使用例**
GPIO2 を出力モードに設定する場合は `[0x02, 0x01]` を送信します。GPIO2 を HIGH にする場合は `[0x02, 0x0B]` を送信します。
