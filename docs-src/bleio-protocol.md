...
| 4 | PATTERN_FLICKER1 | 炎のゆらめきパターン (GPIO 共有明度、色相ゆらぎなし) |
| 5 | PATTERN_FLICKER2 | 炎のゆらめきパターン (LED 個別明度、色相ゆらぎなし) |
| 6 | PATTERN_FLICKER3 | 炎のゆらめきパターン (LED 個別明度、色相ゆらぎあり) |
...
- **PATTERN_RAINBOW (3)**:
  - Param3: 色相が一周する LED 個数 (1-16、0 を与えると デフォルト: 12 として扱う)
  ...
  - Param4: 変化スピード (0-255)
    ...
    - 128: 約 0.64秒で一周 (中速)
    - 255: 約 0.32秒で一周 (高速)
- **PATTERN_FLICKER1 (4)**, **PATTERN_FLICKER2 (5)**, **PATTERN_FLICKER3 (6)**:
  - Param3: ゆらめきの速度 (0-255)
    - 0: 約 1秒周期で変化 (ゆっくり)
    - 64: 約 0.6秒周期で変化 (やや遅い)
    - 128: 約 0.15秒周期で変化 (中速)
    - 192: 約 0.1秒周期で変化 (やや速い)
    - 255: 約 0.05秒周期で変化 (速い)
  - Param4: ゆらめきの変化幅 (0-255)
    - 0: 変動なし (基準色のまま)
    - 64: 小さい変動 (控えめなゆらめき)
    - 128: 中程度の変動 (自然なゆらめき)
    - 192: 大きい変動 (激しいゆらめき)
...
- **色相オフセット**: 各 LED は、LED 番号に応じた色相オフセットを持ちます (LED 番号 × 65536 / Param3)
- **同期動作**: 同じ GPIO の複数の LED は、共通の基準クロックを共有し、同期してアニメーションします
- **更新周期**: パターンは 10[ms] 周期で更新されます
- **色変換**: HSV (色相、彩度 100%、明度 = 基準輝度) から RGB に変換され、ガンマ補正 (γ=2.6) が適用されます

**FLICKER パターンの動作詳細**

FLICKER パターンは、炎のゆらめきを表現するパターンです。SET_OUTPUT_SERIALLED_BASECOLOR で設定した基準色を中心に、明度をゆらめかせます。3 つの種別があり、用途に応じて使い分けます。
...
- **パラメータ共有**: 同じ GPIO の FLICKER1 LED すべてが速度 (Param3) と変化幅 (Param4) を共有します。最後に処理された FLICKER1 LED のパラメータが GPIO 全体に適用されます
- **推奨設定**: 同一 GPIO の FLICKER1 LED には同じ Param3 と Param4 を設定することを推奨します
- **更新周期**: パターンは 10[ms] 周期で更新されます
...
- **速度パラメータ (Param3)**: 値が小さいほどゆっくり、大きいほど速くゆらめきます (デフォルト: 128)
  - FLICKER1: GPIO 全体で共有 (最後に処理された値を使用)
  - FLICKER2/3: LED ごとに独立
- **変化幅パラメータ (Param4)**: 値が小さいほど控えめ、大きいほど激しくゆらめきます (デフォルト: 128)
  - FLICKER1: GPIO 全体で共有 (最後に処理された値を使用)
  - FLICKER2/3: LED ごとに独立
- **間欠カオス法**: 間欠カオス法による自然な 1/f ゆらぎを実現
