...
可変長配列 (最小 5 バイト、最大 245 バイト)

**全体構造**

| バイト位置 | 名称 | 型 | 説明 |
|----------|------|-----|------|
| 0 | Command Count | uint8 | コマンド個数 (1-61) |
...
| 2 | Param1 | uint8 | パラメータ1 (将来用、現在は 0x00) |
...
可変長配列 (最小 1 バイト、最大 123 バイト)

**全体構造**

| バイト位置 | 名称 | 型 | 説明 |
|----------|------|-----|------|
| 0 | Input Count | uint8 | 入力ピン数 (0-61) |
...
- 最大: 1 + 2 × 61 = 123 バイト (入力ピンが 61 個の場合)
...
espio-server は BLE の ATT MTU (Maximum Transmission Unit) を 247 バイトに設定しています。これにより、1回の送信で最大 61 個のコマンドを送信できます。

**パケット長の計算**

- パケット長 = 1 (コマンド個数) + 4 × N (各コマンド)
- 最小: 1 + 4 = 5 バイト (1コマンド)
- 最大: 1 + 4 × 61 = 245 バイト (61コマンド)

**互換性**

クライアントが MTU 247 バイトをサポートしていない場合、デフォルトの MTU (23 バイト、実効 20 バイト) が使用されます。この場合、最大 4 コマンドまで送信できます。
...
各コマンドの Param1 と Param2 を活用することで、追加のデータ長変更なしに PWM 機能を実装できます。
